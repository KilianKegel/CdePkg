...
@brief
    This module implements <em>POST diagnostic for GPIO</em>
@todo

@mainpage
    POST diagnostic for GPIO

@section intro_sec Introduction
    POST diagnostic for GPIO  sample implementation
...
@subsection Drv_sec Driver and Application matrix
    <table>
                ...
                <tr><th>GUID</th>
                    <TD>CDE000FF-4815-4A63-86AF-052FB39671BB</td>
                    <TD>CDE000FF-077B-4A26-BC22-F0329B21F9F9</td>
                    <TD>CDE000FF-FF24-4FB7-B7C2-D4FB22D4624F</td>
    </table>

@subsection ref_sec References
    <a href="https://www.mouser.com/datasheet/2/612/atom-e3800-family-datasheet-1522396.pdf#page=4569">GPIO</a>\n
    <a href="https://www.pdf-archive.com/2014/10/02/ansi-iso-9899-1990-1/ansi-iso-9899-1990-1.pdf">ANSI C Specification</a>\n

@subsection Parm_sec Command line parameters
    1. no command line parameter -> register dump of all GPIO
    2. /all -> dump all GPIO and decode bitfields
...
int chkcmdln(int argc, char** argv) {
    ...
    if (0 != ihlp)
    {
        CDEMOFINE((MFNINF(1)"    BayTrailGPIOSpy w/o parameter - dump all GPIO\n"));
        CDEMOFINE((MFNINF(1)"    BayTrailGPIOSpy /list         - list GPIO names supported by this program\n"));
        CDEMOFINE((MFNINF(1)"    BayTrailGPIOSpy /all          - displays current settings of all GPIO\n"));
        CDEMOFINE((MFNINF(1)"    BayTrailGPIOSpy gpio_name     - displays current settings of one particular GPIO\n"));
        CDEMOFINE((MFNINF(1)"    NOTE: to change GPIO settings during runtime, use a memory write tool.\nThe memory address of a given GPIO is provided by this tool.\n\n"));
        nRet = 1;
    }
    CDEMOFINE((MFNINF(1) "\n"));
    return nRet;
...
int main(int argc, char** argv) {
    ...
    CDEMOFINE((MFNINF(1) "gpiobase -> %08X\n", gpiobase));
    ...
    do {
        ...
        if ((argc > 1) && 0 == strcmp("/all", argv[1])) {
            CDEMOFINE((MFNINF(1)"all!, ELC == %d\n", (int)ELC(GpioID2Name)));
            fPrintAll = 1;
        }
        //
        ...
        if (1 == fPrintAll || argc > 1) {
            ...
            for (i = 0; i < ELC(GpioID2Name); i++) {
                ...
                if (1 == fPrintAll || 0 == strcmp(GpioID2Name[i].szGpioName, argv[1])) {
                    ...
                    CDEMOFINE((MFNBAR(1)"%s %p: %08X %08X %08X %08X\n", GpioID2Name[i].szGpioName, pGpio, gpiobuf.cfg.reg, gpiobuf.mpx.reg, gpiobuf.val.reg, gpiobuf.tst.reg));

                    CDEMOFINE((MFNBAR(1)"cfg.func_pin_mux       2:0: 0x%X       val.pad_val              0: 0x%X\n", gpiobuf.cfg.bit.func_pin_mux, gpiobuf.val.bit.pad_val));
                    CDEMOFINE((MFNBAR(1)"cfg.RSVD0              6:3: 0x%X       val.ioutenb              1: 0x%X\n", gpiobuf.cfg.bit.RSVD0, gpiobuf.val.bit.ioutenb));
                    CDEMOFINE((MFNBAR(1)"cfg.pull_assign        8:7: 0x%X       val.iinenb               2: 0x%X\n", gpiobuf.cfg.bit.pull_assign, gpiobuf.val.bit.iinenb));
                    CDEMOFINE((MFNBAR(1)"cfg.pull_str          10:9: 0x%X       val.RSVD              31:3: 0x%X\n", gpiobuf.cfg.bit.pull_str, gpiobuf.val.bit.RSVD));
                    CDEMOFINE((MFNBAR(1)"cfg.bypass_flop         11: 0x%X       \n", gpiobuf.cfg.bit.bypass_flop));
                    CDEMOFINE((MFNBAR(1)"cfg.RSVD1               12: 0x%X       mpx.dll_std_mux        4:0: 0x%X\n", gpiobuf.cfg.bit.RSVD1, gpiobuf.mpx.bit.dll_std_mux));
                    CDEMOFINE((MFNBAR(1)"cfg.ihysctl          14:13: 0x%X       mpx.dll_hgh_mux        9:5: 0x%X\n", gpiobuf.cfg.bit.ihysctl, gpiobuf.mpx.bit.dll_hgh_mux));
                    CDEMOFINE((MFNBAR(1)"cfg.RSVD2               15: 0x%X       mpx.dll_ddr_mux      14:10: 0x%X\n", gpiobuf.cfg.bit.RSVD2, gpiobuf.mpx.bit.dll_ddr_mux));
                    CDEMOFINE((MFNBAR(1)"cfg.fast_clkgate        16: 0x%X       mpx.dll_cf_od           15: 0x%X\n", gpiobuf.cfg.bit.fast_clkgate, gpiobuf.mpx.bit.dll_cf_od));
                    CDEMOFINE((MFNBAR(1)"cfg.slow_clkgate        17: 0x%X       mpx.RSVD             31:16: 0x%X\n", gpiobuf.cfg.bit.slow_clkgate, gpiobuf.mpx.bit.RSVD));
                    CDEMOFINE((MFNBAR(1)"cfg.filter_slow         18: 0x%X       \n", gpiobuf.cfg.bit.filter_slow));
                    CDEMOFINE((MFNBAR(1)"cfg.filter_en           19: 0x%X       tst.RSVD0              9:0: 0x%X\n", gpiobuf.cfg.bit.filter_en, gpiobuf.tst.bit.RSVD0));
                    CDEMOFINE((MFNBAR(1)"cfg.debounce            20: 0x%X       tst.bscan_bypass         1: 0x%X\n", gpiobuf.cfg.bit.debounce, gpiobuf.tst.bit.bscan_bypass));
                    CDEMOFINE((MFNBAR(1)"cfg.RSVD3            22:21: 0x%X       tst.RSVD1            31:11: 0x%X\n", gpiobuf.cfg.bit.RSVD3, gpiobuf.tst.bit.RSVD1));
                    CDEMOFINE((MFNBAR(1)"cfg.RSVD4               23: 0x%X       \n", gpiobuf.cfg.bit.RSVD4));
                    CDEMOFINE((MFNBAR(1)"cfg.gd_level            24: 0x%X       \n", gpiobuf.cfg.bit.gd_level));
                    CDEMOFINE((MFNBAR(1)"cfg.gd_tpe              25: 0x%X       \n", gpiobuf.cfg.bit.gd_tpe));
                    CDEMOFINE((MFNBAR(1)"cfg.gd_tne              26: 0x%X       \n", gpiobuf.cfg.bit.gd_tne));
                    CDEMOFINE((MFNBAR(1)"cfg.direct_irq_en       27: 0x%X       \n", gpiobuf.cfg.bit.direct_irq_en));
                    CDEMOFINE((MFNBAR(1)"cfg.RSVD5               28: 0x%X       \n", gpiobuf.cfg.bit.RSVD5));
                    CDEMOFINE((MFNBAR(1)"cfg.disable_second_mask 29: 0x%X       \n", gpiobuf.cfg.bit.disable_second_mask));
                    CDEMOFINE((MFNBAR(1)"cfg.RSVD6               30: 0x%X       \n", gpiobuf.cfg.bit.RSVD6));
                    CDEMOFINE((MFNBAR(1)"cfg.RSVD7               31: 0x%X       \n\n", gpiobuf.cfg.bit.RSVD7));
                }
            }
            break;
        }
        ...
        for (i = 0; i < ELC(GpioID2Name); i++) {
            ...
            CDEMOFINE((MFNBAR(1)"%20s %p: %08X %08X %08X %08X\n", GpioID2Name[i].szGpioName, pGpio, gpiobuf.cfg.reg, gpiobuf.mpx.reg, gpiobuf.val.reg, gpiobuf.tst.reg));
        }
